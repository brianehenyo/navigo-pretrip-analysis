...
```{r}
1-pchisq(12.23704,45)

summary(mod.pc.data)

w <- llbt.worth(mod.pc.data)
colnames(w) <- c("Neither", "Auto↓", "Auto↑")
plot(w, log = "y", ylab = "Estimated Worth")
```

```{r}
conditions <- c("CP", "VP", "FP", "CR", "VR", "FR", "BL")
mod.pc.data.all <- llbtPC.fit(pc.data, nitems = 7, obj.names = conditions)
mod.pc.data.all
...
p.hl.intrinsic <- df.hl.consolidated.mvs.long %>% 
  ...
  ggplot(aes(x = variable, y = value, label = condition, shape = condition, group = condition)) +
    ...
    labs(x = "",
         y = "Worth Estimate") +
    scale_y_continuous(breaks = seq(0, 0.45, by = 0.05), limits = c(0, 0.4)) +
    theme_tufte(base_family = "Arial") +
    theme(plot.title = element_text(hjust = 0.5))
